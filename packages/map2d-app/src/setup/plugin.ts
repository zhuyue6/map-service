...
const pluginsMap = {
  ...
  scale: createScalePlugin(),
  rank: createRankPlugin()
};
...
export function dependPluginRegister(
  app: App,
  plugin: Plugin,
...
export function register(app: App, options?: PluginOptions[]) {
  ...
  // 先使用传入插件，没有传入再使用默认插件
  const pluginOptionsList =
    options ??
    initElementsPluginsOptionsList.concat(initToolsPluginsOptionsList);
